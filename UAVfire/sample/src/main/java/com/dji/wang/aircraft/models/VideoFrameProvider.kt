...
object VideoFrameProvider {
    ...
    /** 帧序号：每设置一帧 latestJpeg 就 +1，供检测线程判断是否有新帧 */
    @Volatile
    var frameSeq: Long = 0
        private set
    ...
    private fun yuvToJpeg(yuvData: ByteArray, format: Int, width: Int, height: Int): ByteArray? {
        return try {
            ...
            // 需缩放时先用高质量编码中间图，避免「压缩→缩放→再压缩」双重损失
            val intermediateQuality = if (width > MAX_WIDTH) 100 else JPEG_QUALITY
            yuvImage.compressToJpeg(Rect(0, 0, width, height), intermediateQuality, jpegBos)
