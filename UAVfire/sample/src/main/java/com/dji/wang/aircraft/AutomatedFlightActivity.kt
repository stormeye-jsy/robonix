...
class AutomatedFlightActivity : AppCompatActivity() {
    ...
    private var firePatrol: FirePatrolController? = null
    private var fireOverlayBitmap: Bitmap? = null
    private var firePatrolActive = false
    ...
    private fun initViews() {
        ...
        binding.btnModeCruise.setOnClickListener {
            if (automatedFlightVM.isCruisePaused.value == true) {
                automatedFlightVM.resumeCruiseFromManual()
            } else {
        ...
        binding.btnModeManual.setOnClickListener {
            if (automatedFlightVM.isCruiseActive.value == true) {
                automatedFlightVM.pauseCruiseToManual()
            } else {
        ...
        // 火情检测开关
        binding.btnFireDetect.setOnClickListener { toggleFirePatrol() }
    }

    // ---- 火情检测 ----

    private fun initFirePatrol() {
        firePatrol = FirePatrolController(
            context = this,
            vm = automatedFlightVM,
            onFrame = { bmp ->
                if (!firePatrolActive) {
                    bmp.recycle()
                } else {
                    binding.imageVideo.setImageBitmap(bmp)
                    fireOverlayBitmap?.recycle()
                    fireOverlayBitmap = bmp
                }
            },
            onStatus = { text -> binding.textFireStatus.text = text }
        )
    }

    private fun toggleFirePatrol() {
        val patrol = firePatrol ?: return
        if (firePatrolActive) {
            patrol.stop()
            firePatrolActive = false
            binding.btnFireDetect.text = "🔥 火检"
            binding.btnFireDetect.setTextColor(0xFFFF6D00.toInt())
            binding.textFireStatus.text = "已关闭"
            binding.imageVideo.setImageBitmap(null)
            fireOverlayBitmap?.recycle()
            fireOverlayBitmap = null
        } else {
            firePatrolActive = true
            binding.btnFireDetect.text = "⏹ 火检中"
            binding.btnFireDetect.setTextColor(0xFFF44336.toInt())
            binding.textFireStatus.text = "正在加载模型..."
            patrol.start()
        }
    }
    ...
    private fun observeViewModel() {
        ...
        automatedFlightVM.missionState.observe(this) { state ->
            ...
            // 模式按钮：手动操控/自动巡航在巡航与悬停中也可用（用于暂停/继续）
            val cruising = automatedFlightVM.isCruiseActive.value == true
            val isPaused = automatedFlightVM.isCruisePaused.value == true
            binding.btnModeStandby.isEnabled = isIdle
            binding.btnModeCruise.isEnabled = isIdle || isPaused
            binding.btnModeManual.isEnabled = isIdle || cruising || isPaused
